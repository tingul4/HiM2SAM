# CUDA_VISIBLE_DEVICE=2 python scripts/run_skitb_inference_v2.py
import cv2
import gc
import numpy as np
import os
...
def load_boxes(gt_path):
    """讀取 boxes.txt 的所有行，返回一個列表，每個元素是 [x, y, w, h]"""
...
def load_cameras(cam_path):
    """讀取 cameras.txt，返回一個列表，每個元素是 camera ID"""
...
def calculate_mask_confidence(mask):
    """計算 mask 的品質分數 (連通性 + 大小)"""
    if mask is None or mask.sum() == 0:
        return 0.0
    
    mask_area = mask.sum()
    total_area = mask.size
    fill_ratio = mask_area / total_area
    return fill_ratio
...
def main():
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        max_frames_to_search_ahead = 60  # 2 seconds at 30 fps
            ...
            for frame_idx, object_ids, masks in prop_gen:
                ...
                for obj_id, mask in zip(object_ids, masks):
                    ...
                    if len(non_zero_indices) > 0:
                        ...
                        last_valid_bbox = [bbox_x, bbox_y, bbox_w, bbox_h]
                        frames_since_last_valid = 0
                        mask_found = True
                ...
                if next_frame_idx < max_safe_idx:
                    ...
                    if next_cam != curr_cam:
                        ...
                        logger.info("Continuing propagation to use HiM2SAM's long-term memory...")
                        
                        # 不重置狀態！讓 SAM2 根據長期記憶繼續追蹤
                        # 我們改用 "Look-Ahead" 策略：監控 mask 品質，一旦在新場景找到有效 mask，反向追蹤到場景切換點
                        
                        # 先標記這是場景切換的起始點
                        camera_switch_frame = frame_idx
                        search_start_frame = next_frame_idx
                        search_end_frame = min(max_safe_idx, next_frame_idx + max_frames_to_search_ahead)
                        
                        found_recovery_frame = -1
                        recovery_frame_masks = None
                        
                        # 暫停當前迴圈，進行先行查看
                        logger.info(f"Look-ahead search from frame {search_start_frame} to {search_end_frame}...")
                        
                        # 臨時生成器來掃描後續幀
                        temp_gen = predictor.propagate_in_video(
                            state, 
                            start_frame_idx=search_start_frame, 
                            max_frame_num_to_track=(search_end_frame - search_start_frame)
                        )
                        
                        for temp_frame_idx, temp_obj_ids, temp_masks in temp_gen:
                            # 檢查是否有有效 mask
                            if temp_obj_ids and len(temp_masks) > 0:
                                temp_mask = temp_masks[0][0].cpu().numpy() > 0.0
                                if temp_mask.sum() > 50:  # 有足夠像素的 mask
                                    found_recovery_frame = temp_frame_idx
                                    recovery_frame_masks = temp_masks
                                    logger.info(f"Found valid mask at frame {temp_frame_idx} via long-term memory")
                                    break
                        
                        if found_recovery_frame != -1:
                            # 找到恢復點，現在反向追蹤填補缺口
                            logger.info(f"Reverse tracking from frame {found_recovery_frame} to {search_start_frame}...")
                            
                            # 反向追蹤
                            backward_results = {}
                            frames_to_backtrack = found_recovery_frame - search_start_frame
                            
                            if frames_to_backtrack > 0:
                                back_gen = predictor.propagate_in_video(
                                    state,
                                    start_frame_idx=found_recovery_frame,
                                ...
                                for b_frame_idx, b_obj_ids, b_masks in back_gen:
                                    ...
                                    b_mask = None
                                    if b_masks is not None and len(b_masks) > 0 and b_masks[0] is not None and len(b_masks[0]) > 0:
                                        b_mask = (b_masks[0][0].cpu().numpy() > 0.0)
                                    
                                    if b_mask is not None and b_mask.sum() > 0:
                            ...
                            # 寫入補全的幀
                            for gap_idx in range(search_start_frame, found_recovery_frame):
                            ...
                            # 設定下一個處理位置為恢復點之後
                            current_proc_frame = found_recovery_frame
                        else:
                            # 無法恢復，寫入空幀
                            logger.warning("Could not recover tracking after camera switch. Writing empty frames...")
                            for empty_idx in range(search_start_frame, search_end_frame):
                                if empty_idx < max_safe_idx:
                                    bbox_file.write("0,0,0,0\n")
                                    empty_img = cv2.imread(os.path.join(video_frames_dir, frame_names[empty_idx]))
                                    out.write(empty_img)
                                    pbar.update(1)
                            current_proc_frame = search_end_frame
                        
                        break  # 打破內部 for 迴圈，重新開始傳播
